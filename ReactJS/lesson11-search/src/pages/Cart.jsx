...
const Cart = () => {
    ...
    const clearAll =()=>{
        swal({
            title: "Are you sure?",
            text: "Your all products will delete!",
            icon: "warning",
            buttons: true,
            dangerMode: true,
          })
          .then((willDelete) => {
            if (willDelete) {
              swal("Your all products has been deleted!", {
                icon: "success",
              });
              emptyCart();
            } else {
              swal("Your all products is safe!");
            }
          });
    }
    return (
        <>
            {isEmpty ? <div className='d-flex justify-content-center align-items-center'><img src="https://silkbooth.com/media/wysiwyg/test/Cart_empty_page.gif" alt="" /></div> : <div className='d-flex align-items-center justify-content-center flex-column'>
                ...
                <button className='btn btn-dark mt-3' onClick={()=>{clearAll()}}>All Clear</button>
            </div>}
        </>
    )
}
